...
function add_users_and_deliveries( $column, $post_id ) {
	switch ( $column ) {
		...
		case 'wmc-player':
			...
			$player_id = $order ? $order->get_meta( 'player_id' ) : '';
...
function get_count( $order_id, $type = '' ) {
	...
	$search_key = ( 'delivered' === $type ) ? '_wmc_delivered' : '_wmc_commands';
	$found = array();

	foreach ( $order->get_meta_data() as $meta ) {
		if ( false !== stripos( $meta->key, $search_key ) ) {
			$found[] = $meta->value;
		}
	}
...
function do_resend_donations_field( $order ) {
	...
	$player_id = $order->get_meta( 'player_id' );
	$servers   = get_option( WM_SERVERS );
	$meta_data = $order->get_meta_data();

	if ( empty( $player_id ) || empty( $meta_data ) ) {
...
function admin_scripts( $hook = '' ) {
	...
	if ( 'post.php' === $hook ) {
		...
		if ( isset( $post->ID ) ) {
			...
			$script_data['player_id'] = $order ? $order->get_meta( 'player_id' ) : '';
		}
	}
